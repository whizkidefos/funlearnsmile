...
<?php
$paypal_client_id = get_theme_mod( 'paypal_client_id', 'YOUR_PAYPAL_CLIENT_ID' );
$paypal_currency = get_theme_mod( 'paypal_currency', 'GBP' );
$paypal_mode = get_theme_mod( 'paypal_mode', 'live' );

if ( $paypal_client_id && $paypal_client_id !== 'YOUR_PAYPAL_CLIENT_ID' ) :
?>
<script src="https://www.paypal.com/sdk/js?client-id=<?php echo esc_attr( $paypal_client_id ); ?>&currency=<?php echo esc_attr( $paypal_currency ); ?><?php echo ( $paypal_mode === 'sandbox' ? '&enable-funding=venmo&buyer-country=US' : '' ); ?>"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    let selectedAmount = 10;
    const currency = '<?php echo esc_js( $paypal_currency ); ?>';
            ...
            return actions.order.capture().then(function(details) {
                ...
                fetch('<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>', {
                    ...
                    body: new URLSearchParams({
                        action: 'funlearnsmile_log_donation',
                        nonce: '<?php echo wp_create_nonce( 'funlearnsmile_donation_nonce' ); ?>',
                ...
                .then(result => {
                    ...
                    const message = `Thank you for your generous donation, ${donorName}! Your support means the world to us. ❤️`;
                    
                    // Create success modal
                    const modal = document.createElement('div');
                    modal.className = 'fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4';
                    modal.innerHTML = `
                        <div class="bg-white rounded-3xl p-8 max-w-md text-center animate-on-scroll">
                            <div class="w-20 h-20 bg-grass-green/10 rounded-full flex items-center justify-center mx-auto mb-6">
                                <svg class="w-10 h-10 text-grass-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                            </div>
                            <h3 class="font-fredoka font-bold text-2xl text-deep-blue mb-4">Donation Successful!</h3>
                            <p class="font-nunito text-gray-700 mb-6">${message}</p>
                            <button onclick="window.location.href='<?php echo esc_url( home_url() ); ?>'" class="btn btn-primary">Return Home</button>
                        </div>
                    `;
...
<?php else : ?>
<div class="bg-bright-yellow/20 border-l-4 border-bright-yellow p-6 rounded-lg">
    <p class="font-nunito text-gray-800">
        <strong>PayPal not configured.</strong> Please add your PayPal Client ID in 
        <a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>" class="text-sky-blue hover:text-soft-coral underline">
            Appearance → Customize → PayPal Settings
        </a>
    </p>
</div>
<?php endif; ?>
